...
class Slots {
    ...
    public static function Spin($userID, $gameType, $bet, $windowID) {
        ...
        try { DB::BeginTransaction();
            ...
            if ($prizeID != null) {
                ...
                $result['last_win'] = $prizeData['payout_winnings'];
            }
            ...
            if ($prizeID != null) {
                ...
                self::RecordDayWinnings($userID, $prizeData['payout_winnings']);
    ...
    public static function GetDayWinnings($userID) {
        $date = date('Y-m-d');
        $winnings = (float)DB::Scalar(
            "SELECT day_winnings FROM " . Config::TABLE_PREFIX . "slots_day_winnings
			   WHERE user_id = " . DB::DQ($userID) . " AND date = '" . DB::DQ($date) . "';");
    ...
    private static function RecordDayWinnings($userID, $winnings) {
        // Record day winnings
        $date = date('Y-m-d');
        $id = DB::Scalar(
            "SELECT id FROM " . Config::TABLE_PREFIX . "slots_day_winnings
			   WHERE user_id = " . DB::DQ($userID) . " AND date = '" . DB::DQ($date) . "';");

        if (!$id) {
            DB::Execute(
                "INSERT INTO " . Config::TABLE_PREFIX . "slots_day_winnings
			   (user_id, date, day_winnings) VALUES (" . DB::DQ($userID) . ", '" . DB::DQ($date) . "', 0);");
            $id = DB::InsertID();
        }

        DB::Execute(
            "UPDATE " . Config::TABLE_PREFIX . "slots_day_winnings
			   SET day_winnings =  day_winnings + " . DB::DQ($winnings) . "
			   WHERE id = " . DB::DQ($id) . ";");
    }
}
